...
function MOVE(from,to,captured,promoted,flag) {
	// vi behöver skifta tillbaka det vi gjorde i defs.js med TOSQ,CAPTURED, PROMOTED; inget behövs göras med from och flag
...
function GenerateMoves() {
	...
	for (index = brd_moveListStart[1]; index < brd_moveListStart[2]; ++index)
		move = moveList[index];

	... use move

	brd_moveListStart[2] = brd_moveListStart[1];
	AddMove(Move){
	brd_moveList[brd_moveListStart[2]]= Move;
	brd_moveListStart[2]++;
	...
	if(brd_side == COLOURS.WHITE) {
		...
		for(pceNum = 0; pceNum < brd_pceNum[pceType]; ++pceNum) {
			...
			if(brd_pieces[sq + 10] == PIECES.EMPTY) {
				...
				if(RowBrd[sq] == ROWS.ROW_2 && brd_pieces[sq + 20] == PIECES.EMPTY) {
		...
		for(pceNum = 0; pceNum < brd_pceNum[pceType]; ++pceNum) {
			...
			if(brd_pieces[sq - 10] == PIECES.EMPTY) {
				...
				if(RowBrd[sq] == ROWS.ROW_7 && brd_pieces[sq - 20] == PIECES.EMPTY) {
					AddQuietMove(MOVE(sq,(sq-20),PIECES.EMPTY,PIECES.EMPTY,MFLAGPS));
				}
			}
